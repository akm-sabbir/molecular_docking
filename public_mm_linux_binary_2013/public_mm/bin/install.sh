...
cd $BASEDIR
# Lexicon files
FACTS=`find ./lexicon/morph -name \*facts -print | wc -l`
if [ $FACTS == 0 ]; then
  echo "Warning: lexicon facts files are missing, cannot ensure correct operation of MetaMap without them!"
  echo "Warning: lexicon facts files are missing, cannot ensure correct operation of MetaMap without them!" >> ./install.log
  echo ""
  MISSINGFILES=`expr $MISSINGFILES + 1`
fi
RULES=`find ./lexicon/morph -name \*rules -print | wc -l`
if [ $RULES == 0 ]; then
  echo "Warning: lexicon rules files are missing, cannot ensure correct operation of MetaMap without them!"
  echo "Warning: lexicon rules files are missing, cannot ensure correct operation of MetaMap without them!" >> ./install.log
  echo ""
  MISSINGFILES=`expr $MISSINGFILES + 1`
fi
LEXFILE=`find ./lexicon/data -name lexiconStatic\?\?\?\? -print | wc -l`
if [ $LEXFILE == 0 ]; then
  echo "Warning: lexicon lookup files are missing, cannot ensure correct operation of MetaMap without them!"
  echo "Warning: lexicon lookup files are missing, cannot ensure correct operation of MetaMap without them!" >> ./install.log
  echo ""
  MISSINGFILES=`expr $MISSINGFILES + 1`
else 
  if [ $OS = "MINGW32_NT-5.1" ]; then
      echo "Trying to fix any lexicon text files that may have been converted from lf to crlf..."
      find $BASEDIR/lexicon/data -name lexiconStatic\?\?\?\? -print -exec dos2unix -U {} \;
  fi
fi
LEXIDXS=`find ./lexicon/data -name lexiconStatic\*IndBy\*.dbx -print | wc -l`
if [ $LEXIDXS == 0 ]; then
  echo "Warning: lexicon index files are missing, cannot ensure correct operation of MetaMap without them!"
  echo "Warning: lexicon index files are missing, cannot ensure correct operation of MetaMap without them!" >> ./install.log
  echo ""
  MISSINGFILES=`expr $MISSINGFILES + 1`
fi

# MetaMap DB files
DBBASEFS=`find ./DB -name DB.\*.base -print | wc -l`
if [ $DBBASEFS == 0 ]; then
  echo "Warning: UMLS index files are missing, cannot ensure correct operation of MetaMap without them!"
  echo "Warning: UMLS index files are missing, cannot ensure correct operation of MetaMap without them!" >> ./install.log
  echo ""
  MISSINGFILES=`expr $MISSINGFILES + 1`
fi
DBMODELFS=`find ./DB -name DB.\*.[smr]\* -print | wc -l`
if [ $DBMODELFS == 0 ]; then
  echo "Warning: UMLS index files are missing, cannot ensure correct operation of MetaMap without them!"
  echo "Warning: UMLS index files are missing, cannot ensure correct operation of MetaMap without them!" >> ./install.log
  echo ""
